...
const LiveCamera = () => {
  ...
  const captureImage = async () => {
    ...
    canvas.toBlob(async (blob) => {
      setIsAnalyzing(true);
      try {
        const data = await analyzeEmotion(blob);
        ...
        <div className="flex flex-col items-center mb-6">
          ...
          <div className="w-full text-center mt-4">
            ...
            <div className="mt-6">
              ...
              <motion.div
                className="w-full bg-gray-300 rounded-full mt-2 overflow-hidden h-8 flex items-center justify-center"
                animate={{ scale: [1, 1.05, 1] }}
                transition={{ repeat: Infinity, duration: 1.5 }}
              >
                <div
                  className={`w-full h-8 rounded-full flex items-center justify-center text-lg font-bold ${
                    stressLevel === "High" ? "bg-red-600" :
